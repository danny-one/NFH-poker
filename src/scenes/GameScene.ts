...
export class GameScene extends BaseScene {
	...
	private createHelp() {
		...
		this.helpGroup.addMultiple([
			...
			this.add.bitmapText(15, 40, 'arcade', [
				...
				`- Rules modified for NFH Factions`, '',
				``,
				``,
	...
	private createAttract() {
		...
		const poisonvialText = this.add.bitmapText(5, this.scale.gameSize.height - 13, 'arcade', `cloned fr poison vial`, 8);

		const title1 = this.add.bitmapText(this.scale.gameSize.width / 2, 20, 'arcade', 'NFH\n   POKER', 32).setOrigin(0.5, 0);
		const title2 = this.add.bitmapText(this.scale.gameSize.width / 2 + 2, 22, 'arcade', 'NFH\n   POKER', 32).setOrigin(0.5, 0).setTint(Colors.UI_BLUE.color);
		const title3 = this.add.bitmapText(this.scale.gameSize.width / 2 + 4, 24, 'arcade', 'NFH\n   POKER', 32).setOrigin(0.5, 0).setTint(Colors.RED.color);
